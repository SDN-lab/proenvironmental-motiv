...
for imod=1:numel(modnames)
   ...
   if ~isreal(lme(:,imod)) %Should return '0' if any element has imaginary component
       lme(:,imod) = real(lme(:,imod));
       warning('Using real components of complex numbers only');
   end
...
try
compM   = [modnames(bicint == min(bicint)), modnames(bicint == min(setdiff(bicint, min(bicint))))];                                    % models to compare specifically
subplot(2,2,3);

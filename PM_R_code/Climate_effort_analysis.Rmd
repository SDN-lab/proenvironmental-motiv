...
ages <- d.mod.full.wide %>% 
  group_by(country) %>% 
  summarise(mean_age = mean(Age, na.rm = TRUE), min_age = min(Age, na.rm = TRUE), max_age = max(Age, na.rm = TRUE), n =n())
...
demographics <- d.mod.full.wide %>% 
  ...
  mutate(female_per = round(female / n * 100))
...
d.ag.control <- d.ag.full %>% subset(intername == "Control")
d.ag.exp <- d.ag.full %>% subset(intername != "Control")

con.ag.control.bf <- ttestBF(d.ag.control$choice[d.ag.control$agent == "Food"],
                             d.ag.control$choice[d.ag.control$agent == "Climate"], 
                             paired = T)

con.ag.control.bf01 <- 1 / con.ag.control.bf
con.ag.control.bf01 <- extractBF(con.ag.control.bf01)

con.ag.exp.bf <- ttestBF(d.ag.exp$choice[d.ag.exp$agent == "Food"],
                         d.ag.exp$choice[d.ag.exp$agent == "Climate"], 
                         paired = T)

con.ag.exp.bf01 <- 1 / con.ag.exp.bf
con.ag.exp.bf01 <- extractBF(con.ag.exp.bf01)
...
con.k.control.bf <- ttestBF(d.mod.control$k[d.mod.control$agent == "Food"],
                             d.mod.control$k[d.mod.control$agent == "Climate"], 
                             paired = T)

con.k.control.bf01 <- 1 / con.k.control.bf
con.k.control.bf01 <- extractBF(con.k.control.bf01)

con.k.exp.bf <- ttestBF(d.mod.control$k[d.mod.control$agent == "Food"],
                         d.mod.control$k[d.mod.control$agent == "Climate"], 
                         paired = T)

con.k.exp.bf01 <- 1 / con.k.exp.bf
con.k.exp.bf01 <- extractBF(con.k.exp.bf01)
...
```{r beta-controls-models, echo=FALSE, message=FALSE, warning=FALSE, results='asis', cache=TRUE}
...
plot.control.effort <- ggplot(d.tbt.control, aes(x = effort.c,  y = decision.num, fill = agent, color = agent, linetype = agent, group = agent)) +
  ...
  coord_cartesian(ylim = c(0.7,0.91)) +  
...
plot.psydis.effort <- ggplot(d.tbt.exp %>% subset(intername == "PsychDistance"), aes(x = effort.c,  y = decision.num, fill = agent, color = agent, linetype = agent, group = agent)) +
  ...
  scale_linetype_manual(limits=agent.lims, values=agent.lines) +
  scale_x_continuous(breaks = c(2,3,4,5)) + 
  coord_cartesian(ylim = c(0.68,0.91)) +  
  labs(x = "Effort Required", y = choice.lab) +
...
plot.full.effort.country <- plot.full.effort  + 
  coord_cartesian(ylim = c(0.68,0.95)) +
  facet_wrap("country")
...
plot.inter.k.sub <- ggplot(d.mod.exp, aes(x = intername,  y = k_sub, fill = intername, color = intername, group = intername, alpha = intername, linetype = intername, linewidth = intername)) +
  ...
  scale_linewidth_manual(limits = d.ag.full.diff.av$intername, values = c(rep.int(1.5,11))) +
  geom_hline(yintercept = 0, linetype="dashed") +
  labs(y = paste0("Decreased ", k.symbol, " for<br>climate vs. food")) +
  theme_classic() +
  paper.theme +
  theme(legend.position = "none",
        axis.title.y = element_blank(),
...
`r demographics$female_per[demographics$country == "Bulgaria"]`% female), 
Greece
...
`r demographics$female_per[demographics$country == "Greece"]`% female), 
Nigeria
...
`r demographics$female_per[demographics$country == "Nigeria"]`% female), 
Sweden
...
`r demographics$female_per[demographics$country == "Sweden"]`% female), 
UK
...
`r demographics$female_per[demographics$country == "UK"]`% female), 
and USA
...
`r demographics$female_per[demographics$country == "US"]`% female; 
...
*p*`r pvalrf(r.inter.choice.lin$p[r.inter.choice.lin$Parameter == "Cause (Food > Climate) * Negative Emotions"])`; Figure 3 & Figure S2). 
...
plot.country <- cowplot::plot_grid(plot.full.effort.country, legend.fc.long, plot.full.reward.country + theme(legend.position = "none"), nrow = 3, align = "lrbt", labels = "auto", rel_heights = c(1,0.15,1))
...
**Table S2.**
```{r control-choice-results, echo=FALSE, warning=FALSE, message=FALSE, fig.align="center", fig.height=5, fig.width=5}
...
**Table S3.**
```{r full-choice-results, echo=FALSE, warning=FALSE, message=FALSE, fig.align="center", fig.height=5, fig.width=5}
...
**Table S4.**
```{r inter-choice-lin-results, echo=FALSE, warning=FALSE, message=FALSE, fig.align="center", fig.height=5, fig.width=5}
...
**Table S5.**
```{r inter-wept-results, echo=FALSE, warning=FALSE, message=FALSE, fig.align="center", fig.height=5, fig.width=5}
...
**Table S7.**
```{r inter-choice-sq-results, echo=FALSE, warning=FALSE, message=FALSE, fig.align="center", fig.height=5, fig.width=5}
...
**Table S8.**
```{r inter-k-results, echo=FALSE, warning=FALSE, message=FALSE, fig.align="center", fig.height=5, fig.width=5}
...
**Table S9.** 
```{r inter-beta-results, echo=FALSE, warning=FALSE, message=FALSE, fig.align="center", fig.height=5, fig.width=5}
